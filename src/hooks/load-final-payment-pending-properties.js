...
export const useFinalPaymentPendingProperties = (isTxnPending) => {
    ...
    useEffect(() => {
        ...
        const fetchProperties = async () => {
            ...
            const listingIdsPendingSaleHex = await escrowContract.getListingsPendingFinalBuyerPayment(user.userAddress);
            const listingIdsPendingSale = listingIdsPendingSaleHex.map(value => parseInt(value._hex, 16));
            console.log('Listing Ids Pending Sale:', listingIdsPendingSale)
